...
class Posts extends Trongate {
    ...
    function fetch_new_tutorial($input){
        ...
        if (sizeof($input['params']) !== 1) {
            //send the topic to openAi to get a tutorial for Recognition
            $secondary_object = $input['params']['secondary_object'];


            $tutorialOpt = $this->make_Rec_tutorial($primary_object,$secondary_object);
    ...
    function init_gen_com_image(){
                //get some posted data
        $params = file_get_contents('php://input'); //json str
        $posted_param = json_decode($params);

        $update_id = $posted_param->updateId ?? 0;
        $tutorialTopic = $posted_param->joinedMsg ?? '';
        ...
        //generate an comparing image using openAI

        $output_img = $this->generate_com_image($tutorialTopic);
    ...
    function make_Rec_tutorial($object1, $object2){
        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, "https://api.openai.com/v1/completions");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);

        $prompt = 'Create a tutorial that distinguish between two objects in logical way as human does. Give me at least three ways to identify them and address the primary method. Here are the following objects: '.$object1.' '.$object2;
    ...
    function generate_com_image($target_subject){
        $ch = curl_init();
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, "{
            \"model\": \"image-alpha-001\",
            \"prompt\": \"two objects place in positions as they are being compare with a robot in the background looking at them. Two objects are: $target_subject'\",
            \"num_images\":1,
            \"size\":\"512x512\",
            \"response_format\":\"url\"
        }");
